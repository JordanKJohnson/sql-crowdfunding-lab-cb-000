...
"SELECT projects.title,SUM(pledges.amount)-MIN(projects.funding_goal) FROM projects INNER JOIN pledges on projects.id = pledges.project_id GROUP BY projects.id HAVING SUM(pledges.amount) >= projects.funding_goal;"
end
