...
"SELECT users.name,pledges.amount from users INNER JOIN pledges on pledges.user_id = users.id GROUP BY pledges.user_id ORDER BY pledges.amount,users.name desc;"
end
