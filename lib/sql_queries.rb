...
"SELECT projects.title,((SUM(pledges.amount))-(MIN(projects.funding_goal)) as raised FROM projects INNER JOIN pledges on projects.id = pledges.project_id GROUP BY projects.id HAVING raised >= projects.funding_goal;"
end
