...
"SELECT projects.title,((SUM(pledges.amount) as summed)-(projects.funding_goal as funded)) as raised FROM projects INNER JOIN pledges on projects.id = pledges.project_id GROUP BY projects.id HAVING raised >= projects.funding_goal;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name,SUM(pledges.amount) as amt from users INNER JOIN pledges on pledges.user_id = users.id GROUP BY pledges.user_id ORDER BY amt,users.name asc;"
end
